...
DATABASE_USER=$(getSetting 'USER')
DATABASE_PASSWORD=$(getSetting 'PASSWORD')
DATABASE_HOST=$(getSetting 'HOST')
DATABASE_PORT=$(getSetting 'PORT')

if [[ -n "$DATABASE_USER" && -n "$DATABASE_PASSWORD" && -n "$DATABASE_HOST" && -n "$DATABASE_PORT" && -n "$DATABASE_NAME" ]]; then
    ...
    mysqldump -u "$DATABASE_USER" -p"$DATABASE_PASSWORD" -h "$DATABASE_HOST" -P "$DATABASE_PORT" "$DATABASE_NAME" | gzip -c | cat > "$OUTPUT"
    chmod 600 "$OUTPUT"
else
