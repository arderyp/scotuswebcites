    ...
    for(var i=0; i<links.length; i++) {
        ...
        if (showLink){
            ...
            if(hasDatetime){
                ...
                //LC-CUSTOM
                RL_appendHiddenLink(dropDownItem, 'Mementos Near Publication Date: '+ RLPrintDate(datetime), link);
            }
            if(hasMemento || hasOriginal){
                //RL_appendHiddenLink(dropDownItem, 'Get from '+ RLPrintDomainName(memento), memento);
                //LC-CUSTOM
                RL_appendHiddenLink(dropDownItem, 'View Verified Capture From: '+ RLPrintDomainName(memento), memento);
            }
            if(hasOriginal){
                //RL_appendHiddenLink(dropDownItem, 'Get at current date', original);
                //LC-CUSTOM
                RL_appendHiddenLink(dropDownItem, 'Visit live Site', original);
            }
